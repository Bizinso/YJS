<template>
  <div class="listing_screen global_table_liting">
    <h2 class="masterHeading">Additional Charges</h2>
    <!-- Search and Actions -->
    <div class="listing_tab_and_actions mb-3">
      ...
      <div class="listing_actions">
        ...
        <div class="buttonGrid">
          <b-button class="fillBTN" v-if="$can('do', 'add_admin.masters.charges')" @click="openAddSidebar">Add</b-button>
...
<div :class="{ parentBackground: sidebarstatus.filter }">
              ...
              <div class="px-4 py-3 column_sidebar">
                <b-form-group >
                  <div class="image-upload-wrapper text-center">
                    ...
                    <div
                      v-else
                      class="thumbnail-container upload-placeholder"
 ...
 <div class="filter_selected px-4" v-if="inqFilterStatus">
   ...
   <div v-if="loading" class="text-center p-5">
      ...
      <p class="mt-2">Loading Additional Charges...</p>
    </div>
    ...
    <div v-else>
            ...
            >
              <template #head(charges_type_name)>
                <span @click="changeSorting('charges_type_name')">
                  Name <i :class="getSortIcon('charges_type_name')"></i>
                </span>
              </template>
              <template #cell(charges_type_name)="data">
                <span>{{ formattedTitle(data.item.charges_type_name) }}</span>
              </template>

              <template #cell(actions)="data">
                <b-dropdown right text="⋮" variant="link" no-caret toggle-class="p-0">
                    <b-dropdown-item v-if="$can('do', 'edit_admin.masters.charges')" @click="openEditSidebar(data.item)">Edit</b-dropdown-item>
                    <b-dropdown-item v-if="$can('do', 'delete_admin.masters.charges')" @click="openDeleteModal(data.item.id)">Delete</b-dropdown-item>
                </b-dropdown>
                
              </template>
            </b-table>

          <additionalChargesNoData
          ...
          <additionalChargesNoData
            ...
            heading="No Additional Charges Created Yet"
            subheading="You haven't added any additional charges yet. Add your first additional charges to get started."
            :showButton="$can('do', 'add_admin.masters.charges')"
            buttonText="Add Additonal Charges"
    ...
    >
      <div class="sidebar_toolbox woBorder">
        <h6>{{
            isEditMode ? "Update Additional Charge" : "Add Additional Charge"
          }}</h6>
        <div class="sidebar_toolbox_right_side">
          <CloseIcon @click="closeSidebar" />
        </div>
      </div>
      <div class="sidebar_form formMaster p-0">
        <b-form @submit.prevent="handleFormSubmit">
          <div class="p-3 column_sidebar">
            ...
            <label class="required">Charges</label>
                <b-form-input
                  id="input-2"
                  v-model="formData.charges_type_name"
                  placeholder="Enter Charges"
                  @input="removeError('charges_type_name')"
                ></b-form-input>
                   <div class="text-danger" v-if="hasErrors('charges_type_name')">
                {{ getErrors("charges_type_name") }}
              </div>
              </b-form-group>
...
import additionalChargesNoData from "../../../components/noData.vue";
...
const additionalChargeTypeOption = ref([]);
...
onMounted(() => {
  axiosEmployee.get("additionalChargesTypesOptions").then((response) => {
    additionalChargeTypeOption.value = response.data.data;
  });
  fetchAdditionalCharges();
...
const getFilterDisplayValue = (key, value) => {
  if (key === 'type_id') {
    const selectedOption = additionalChargeTypeOption.value.find(opt => opt.id === value);
...
const fields = [
  { key: "charges_type_name", label: "Name", sortable: true },
...
const confirmDelete = async () => {
  try {
    const response = await axiosEmployee.delete(
      `additional-charges/${deleteId.value}`
    );
...
const handleFormSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      await axiosEmployee.put(
        `additional-charges/${formData.value.id}`,
        formData.value
      );
     sidebarstatus.value.add = false;
        closeSidebar();
      ...
      await axiosEmployee.post("additional-charges", formData.value);
     sidebarstatus.value.add = false;
       closeSidebar();
      setTimeout(() => {
...
<style>
.sidebar_main .form-check-input[type=radio]{
