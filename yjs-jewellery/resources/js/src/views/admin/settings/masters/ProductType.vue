<template>
  <div class="listing_screen global_table_liting">
    <h2 class="masterHeading">Product Type</h2>
    ...
    <div class="listing_tab_and_actions mb-3">
      ...
      <div class="listing_actions">
        ...
        <div class="buttonGrid">
          <b-button class="fillBTN" v-if="$can('do', 'add_admin.masters.producttype')" @click="openAddSidebar"
            >Add</b-button
          >
      ...
      <div :class="{ parentBackground: sidebarImport }">
        <div
          class="filter_sidebar sidebar_main"
          :class="[sidebarImport ? 'filter_active' : '']"
        >
          ...
          <div class="sidebar_form">
            <b-form>
                      ...
                      >
                        ...
                        <div class="centerPiece">
                          <div class="innerNames">
                            <h3>{{ bulkFile.name }}</h3>
                            <span
                              >{{ (bulkFile.size / 1024).toFixed(2) }} KB</span
                            >
                          </div>
                        </div>
                        <!-- Remove button -->
                        <button
                          class="btn btn-sm btn-outline-danger removeBox"
              ...
              <div class="sidebarbtn_group">
                <div class="buttonGrid">
                  <b-button type="submit" class="fillBTN" @click="searchFilter"
                    >Upload</b-button
                  >
                  <b-button
                    class="transBTN"
 ...
 <div v-if="loading" class="text-center p-5">
      ...
      <p class="mt-2">Loading product type...</p>
    </div>
    ...
    <b-table responsive="sm"
      ...
      @sort-changed="onSortChanged"
       v-if="$can('do', 'list_admin.masters.producttype') && paginatedData.length > 0" 
    >
      <template #head(name)>
      ...
      <template #cell(actions)="data">
        <b-dropdown right text="⋮" variant="link" no-caret toggle-class="p-0">
          <b-dropdown-item v-if="$can('do', 'edit_admin.masters.producttype')" @click="openEditSidebar(data.item)"
            >Edit</b-dropdown-item
          >
    
          <b-dropdown-item v-if="$can('do', 'delete_admin.masters.producttype')" @click="openDeleteModal(data.item.id)"
          ...
          heading="No Product Type Yet"
          subheading="You haven't added any product type yet. Add your first product type to get started."
          :showButton="$can('do', 'add_admin.masters.producttype')"
          buttonText="Add"
    ...
    <div :class="{ parentBackground: sidebarstatus.add }">
      ...
      >
        <div class="sidebar_toolbox woBorder">
          <h6>{{ isEditMode ? "Update Product Type" : "Add Product Type" }}</h6>
          <div class="sidebar_toolbox_right_side">
...
import productTypeNoData from "../../../components/noData.vue";
...
const confirmDelete = async () => {
    try {
        const response = await axiosEmployee.delete(`product-type/${deleteId.value}`);
...
const handleFormSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      await axiosEmployee.put(`product-type/${formData.value.id}`, formData.value);
      sidebarstatus.value.add = false;
        closeSidebar();
        setTimeout(() => {
      ...
      await axiosEmployee.post("product-type", formData.value);
        sidebarstatus.value.add = false;
       closeSidebar();
      setTimeout(() => {
...
const confirmChangeStatus = async () => {
  try {
    const response = await axiosEmployee.post(
      `/product-type/${selectedItem.value.id}/change-status`
    );
