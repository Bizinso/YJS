<template>
    <div class="listing_screen global_table_liting">
        <h2 class="masterHeading">Attribute</h2>
        <!-- Search and Actions -->
        <div class="listing_tab_and_actions mb-3">
            <div class="listing_actions">
                ...
                <div class="buttonGrid">
                    <b-button class="fillBTN" v-if="$can('do', 'add_admin.masters.attributes')"
                        @click="openAddSidebar">Add Attribute</b-button>
                    ...
                    <div class="sidebar_form">
                        <b-form @submit.prevent="searchFilter">
                            <div class="px-4 py-3 column_sidebar">
                                ...
                                <b-form-group>
                                    <b-form-input type="text" v-model="search.name" placeholder="Search by name..." />
                                </b-form-group>
        ...
        <div v-if="loading" class="text-center p-5">
            ...
            <p class="mt-2">Loading attribute...</p>
        </div>
        ...
        <div v-else>
            ...
            <b-table responsive="sm" :items="paginatedData" :fields="fields" @sort-changed="onSortChanged"
                v-if="$can('do', 'list_admin.masters.attributes') && paginatedData.length > 0">
                <template #head(name)>
                    <span @click="changeSorting('name')">
                        Attribute Name <i :class="getSortIcon('name')"></i>
                    </span>
                </template>
                ...
                <template #head(data_type)>
                    <span @click="changeSorting('data_type')">
                        Data Type <i :class="getSortIcon('data_type')"></i>
                    </span>
                </template>
                <template #cell(data_type)="data">
                    {{ formattedTitle(data.item.data_type) }}
                </template>

                <template #head(value)>
                    <span @click="changeSorting('value')">
                        Options <i :class="getSortIcon('value')"></i>
                    </span>
                </template>
                ...
                <template #cell(actions)="data">
                    <b-dropdown right text="⋮" variant="link" no-caret toggle-class="p-0">
                        <b-dropdown-item v-if="$can('do', 'edit_admin.masters.attributes')"
                            @click="openEditSidebar(data.item)">Edit</b-dropdown-item>

                        <b-dropdown-item v-if="$can('do', 'delete_admin.masters.attributes')"
                            @click="openDeleteModal(data.item.id)">Delete</b-dropdown-item>
                    </b-dropdown>
                </template>
            </b-table>

            <attributeNoData v-if="inqFilterStatus && paginatedData.length === 0 || !inqFilterStatus && sidebarstatus.filter" heading="No Records Found"
                subheading="Try adjusting your filters or search criteria." :showButton="false" />

            <attributeNoData v-else-if="paginatedData.length === 0" heading="No Attributes Yet"
                subheading="You haven't added any attributes yet. Add your first attributes to get started."
                :showButton="$can('do', 'add_admin.masters.attributes')" buttonText="Add Attributes"
        ...
        <div :class="{ parentBackground: sidebarstatus.add }">
            ...
            <div class="assigned_sidebar sidebar_main" :class="{ assigned_active: sidebarstatus.add }">
                <div class="sidebar_toolbox woBorder">
                    <h6>{{ isEditMode ? "Update Attribute" : "Add Attribute" }}</h6>
                    <div class="sidebar_toolbox_right_side">
                    ...
                    <b-form @submit.prevent="handleFormSubmit">
                        <div class="p-3 column_sidebar">
                            ...
                            <b-form-group>
                                ...
                                <v-select v-model="formData.data_type" :options="attributeDataType"
                                    :reduce="(val) => val.id" label="name" :clearable="true"
                                    placeholder="Select Data Type" @input="removeError('data_type')" />
                                <div class="text-danger" v-if="hasErrors('data_type')">
                                    {{ getErrors("data_type") }}
                                </div>
                            </b-form-group>
                            ...
                            <b-form-group>
                                ...
                                <b-form-input v-model="formData.value" placeholder="Enter Options"
                                    @input="removeError('value')" />
                                <div class="text-danger" v-if="hasErrors('value')">
                                    {{ getErrors("value") }}
                                </div>
                            </b-form-group>
...
import attributeNoData from "../../../components/noData.vue";
...
const fields = [
    ...
    { key: "value", label: "Options", sortable: true },
...
const openEditSidebar = (item) => {
    ...
    const selectedType = attributeDataType.value.find(
        (opt) => opt.name.toLowerCase() === item.data_type.toLowerCase()
    );

    formData.value = {
        ...item,
        data_type: selectedType ? selectedType.id : null,
...
const confirmDelete = async () => {
    try {
        const response = await axiosEmployee.delete(`attribute/${deleteId.value}`);
...
const handleFormSubmit = async () => {
    ...
    try {
        if (isEditMode.value) {
            await axiosEmployee.put(`attribute/${formData.value.id}`, formData.value);
            ...
            await axiosEmployee.post("attribute", formData.value);
            sidebarstatus.value.add = false;
            closeSidebar();
            setTimeout(() => {
...
const confirmChangeStatus = async () => {
    try {
        const response = await axiosEmployee.post(
            `/attribute/${selectedItem.value.id}/change-status`
        );
...
onMounted(() => {
    axiosEmployee.get("attributeDataTypeOption").then((response) => {
        attributeDataType.value = response.data.data;
    });
    fetchAttribute();
