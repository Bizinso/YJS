<template>
    <div class="listing_screen global_table_liting px-3">
        ...
        <div class="listing_tab_and_actions mb-3">
            <div class="listing_actions">
                ...
                <div class="buttonGrid">
                    <b-button class="fillBTN" v-if="$can('do', 'add_admin.masters.tax')" @click="openAddSidebar">Add</b-button>
                        ...
                        <b-form @submit.prevent="searchFilter">
                            <div class="px-4 py-3 column_sidebar">
                                <b-form-group label="Tag Name">
                                    <b-form-input type="text" v-model="search.tax_name"
                                        placeholder="Search by Name..." />
                                </b-form-group>
                                <b-form-group label="Tax Rate">
                                    <b-form-input type="number" v-model="search.tax_rate" :min="1"
        ...
        <div v-if="loading" class="text-center p-5">
            ...
            <p class="mt-2">Loading tax...</p>
        </div>

        <div v-else>

            <b-table responsive="sm" :items="paginatedData" :fields="fields" @sort-changed="onSortChanged"
                v-if="$can('do', 'list_admin.masters.tax') && paginatedData.length > 0">
                <template #head(tax_name)>
                    <span @click="changeSorting('tax_name')">
                        Tax Name <i :class="getSortIcon('tax_name')"></i>
                    </span>
                </template>
                <template #cell(tax_name)="data">
                    <span>{{ formattedTitle(data.item.tax_name) }}</span>
                </template>

                <template #head(tax_rate)>
                    <span @click="changeSorting('tax_rate')">
                        Tax Rate(%) <i :class="getSortIcon('tax_rate')"></i>
                    </span>
                </template>
                <template #cell(tax_rate)="data">
                    <span>{{ parseFloat(data.item.tax_rate) }}%</span>
                </template>
                ...
                <template #cell(actions)="data">
                    <b-dropdown right text="⋮" variant="link" no-caret toggle-class="p-0">
                        <b-dropdown-item v-if="$can('do', 'edit_admin.masters.tax')"
                            @click="openEditSidebar(data.item)">Edit</b-dropdown-item>
                        <!-- <b-dropdown-item :to="`/tax/${encodeBase64(data.item.id)}`">View</b-dropdown-item> -->
                        <b-dropdown-item v-if="$can('do', 'delete_admin.masters.tax')"
            ...
            <taxNoData v-if="inqFilterStatus && paginatedData.length === 0 || !inqFilterStatus && sidebarstatus.filter" heading="No Records Found"
                subheading="Try adjusting your filters or search criteria." :showButton="false" />

            <taxNoData v-else-if="paginatedData.length === 0" heading="No Tax Yet"
                subheading="You haven't added any tax yet. Add your first tax to get started."
                :showButton="$can('do', 'add_admin.masters.tax')" buttonText="Add Tax" @button-clicked="openAddSidebar" />
        ...
        <div :class="{ parentBackground: sidebarstatus.add }">
            ...
            <div class="assigned_sidebar sidebar_main" :class="{ assigned_active: sidebarstatus.add }">
                <div class="sidebar_toolbox woBorder">
                    <h6> {{ isEditMode ? "Update Tax" : "Add Tax" }}</h6>
                    <div class="sidebar_toolbox_right_side">
                    ...
                    <b-form @submit.prevent="handleFormSubmit">
                        <div class="p-3 column_sidebar">
                            ...
                            <b-form-group>
                                ...
                                <b-form-input v-model="formData.tax_name" placeholder="Enter Tax Name"
                                    @input="removeError('tax_name')" />
                                <div class="text-danger" v-if="hasErrors('tax_name')">
                                    {{ getErrors("tax_name") }}
                                </div>
                            </b-form-group>
                            ...
                            <b-form-group>
                                ...
                                <b-form-input type="number" v-model="formData.tax_rate" :min="1"
                                    placeholder="Enter Tax Rate" @input="removeError('tax_rate')" />
                                <div class="text-danger" v-if="hasErrors('tax_rate')">
                                    {{ getErrors("tax_rate") }}
                                </div>
                            </b-form-group>
...
import taxNoData from "../../../components/noData.vue";
...
const fetchTaxData = () => {
    ...
    return axiosEmployee
        .get(`tax/`, {
            params: {
                tax_name: search.value.tax_name,
                tax_rate: search.value.tax_rate,
...
const confirmDelete = async () => {
    try {
        const response = await axiosEmployee.delete(`tax/${deleteId.value}`);
...
const handleFormSubmit = async () => {
    ...
    try {
        if (isEditMode.value) {
            await axiosEmployee.put(`tax/${formData.value.id}`, formData.value);
            sidebarstatus.value.add = false;
            closeSidebar();
            setTimeout(() => {
            ...
            await axiosEmployee.post("tax", formData.value);
            sidebarstatus.value.add = false;
            closeSidebar();
            setTimeout(() => {
