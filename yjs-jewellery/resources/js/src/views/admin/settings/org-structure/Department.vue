<template>
  <div class="listing_screen global_table_liting">
    <h2 class="masterHeading">Department</h2>
      ...
      <div class="listing_actions">
        <div class="d-flex">
          <div class="listing_search">
            ...
            <b-form-input v-model="search.globalSearch" @input="searchFilter" placeholder="Search Here..." />
          </div>
        ...
        <div class="buttonGrid">
          <b-button class="fillBTN" @click="openAddSidebar"
            v-if="$can('do', 'add_admin.org.departments')">Add</b-button>
          <b-button class="transBTN" v-b-tooltip.hover title="Bulk Upload" @click="sidebarImport = !sidebarImport">
            ...
            <b-form>
              <div class="px-4 py-3 column_sidebar">
                ...
                <b-form-group label="Department Name" label-for="name">
                  <v-select v-model="search.name" :options="DepartmentOption" :reduce="(val) => val.label" label="label"
                    :clearable="true" placeholder="Search Department Name..."></v-select>
                </b-form-group>
                <b-form-group label="Branch Name" label-for="name">
                  <v-select v-model="search.branch_id" :options="BranchOption" :reduce="(val) => val.label"
                ...
                <b-form-group>
                  <div class="image-upload-wrapper text-center">
                    ...
                    <div v-if="bulkFile" class="thumbnail-grid selected-file noRpaoo">
                      <div class="file-info d-flex justify-content-between align-items-center p-2 border rounded gap-2">
                        ...
                        <button class="btn btn-sm btn-outline-danger removeBox" @click="removeBulkFile">✕</button>
                      </div>
                    </div>

                    <!-- Placeholder for upload when no file -->
                    <div v-else class="thumbnail-container upload-placeholder" @click="triggerFileSelect">
    ...
    <div v-else>
      ...
      <div class="filter_selected px-4" v-if="inqFilterStatus">
        ...
        <span class="selected_filter_item" v-if="search.name != null">{{ (search.name) }} <i class="fa-solid fa-xmark"
            @click="() => { search.name = null; searchFilter() }"></i></span>
        <span class="selected_filter_item" v-if="search.branch_id != null">{{ (search.branch_id) }} <i
            class="fa-solid fa-xmark" @click="() => { search.branch_id = null; searchFilter() }"></i></span>
        <span class="selected_filter_item" v-if="search.globalSearch != null">{{ (search.globalSearch) }} <i
            class="fa-solid fa-xmark" @click="() => { search.globalSearch = null; searchFilter() }"></i></span>
      </div>

      <div v-if="selectedDepartmentData.length > 0 || totalrows > 0">
        <b-table responsive="sm" :items="selectedDepartmentData" :fields="fields" @sort-changed="onSortChanged"
          v-if="$can('do', 'list_admin.org.departments')">
          ...
          <template #head(branch_name)="data">
            <span @click="changeSorting('branch_name')">
              Branch Name
              <i :class="getSortIcon('branch_name')"></i>
            </span>
          </template>
          <template #cell(branch_name)="data">
            <span>{{ formattedTitle(data.item.branch_name) }}</span>
          </template>
          ...
          <template #cell(actions)="data">
            ...
            <b-dropdown right text="⋮" variant="link" no-caret toggle-class="p-0">
              <b-dropdown-item @click="openEditSidebar(data.item)"
                v-if="$can('do', 'edit_admin.org.departments')">Edit</b-dropdown-item>

              <b-dropdown-item @click="openDeleteModal(data.item.id)"
                v-if="$can('do', 'delete_admin.org.departments')">Delete</b-dropdown-item>
            </b-dropdown>
        ...
        <div class="tablecounter" v-if="totalrows > 0">
          <div class="show_entries">
            ...
            <v-select append-to-body :calculate-position="withPopper" @update:modelValue="resetPage" v-model="perPage"
      ...
      <departmentNoData v-if="selectedDepartmentData.length === 0 && totalrows === 0"
        :heading="inqFilterStatus ? 'No Records Found' : 'No Department Created Yet'"
        :subheading="inqFilterStatus ? 'Try adjusting your filters or search criteria.' : 'You haven\'t added any Department yet. Add your first Department to get started.'"
        :showButton="!inqFilterStatus && $can('do', 'add_admin.org.departments')"
    ...
    <b-modal id="delete-modal" class="text-center" v-model="deleteModal" hide-header hide-footer size="md" centered>
      <h6 class="mb-3">Are you sure you want to delete this department?</h6>
    ...
    <div :class="{ parentBackground: sidebarstatus.add }">
      ...
      <div class="assigned_sidebar sidebar_main" :class="{ assigned_active: sidebarstatus.add }">
        <div class="sidebar_toolbox woBorder">
          <h6>{{ isEditMode ? 'Update Department' : 'Add Department' }}</h6>
          <div class="sidebar_toolbox_right_side">
          ...
          <b-form @submit.prevent="handleFormSubmit">
            <div class="px-4 py-3 column_sidebar">
              ...
              <div>
                ...
                <b-form-group>
                  <v-select v-model="formData.branch_id" :options="BranchOption" :reduce="(val) => val.value"
                    label="label" :clearable="true" placeholder=" Branch Name..."></v-select>
                </b-form-group>
                <small class="text-danger">{{ errors[0] }}</small>
...
import departmentNoData from "../../../components/noData.vue";
...
const fetchDepartmentOptions = () => {
  axiosEmployee.get("/DepartmentsOption").then((response) => {
    DepartmentOption.value = response.data.data;
  });
};

const getBranchOption = () => {
...
const formData = ref({ name: "", branch_id: "" });
  ...
  axiosEmployee
    .get(`/allDepartments/`, {
      params: {
        ...
        branch_id: search.value.branch_id,
    ...
    .then((response) => {
      selectedDepartmentData.value = response.data.data;
      console.log(selectedDepartmentData.value.length)
      totalrows.value = response.data.total;
      loading.value = false;

      // If current page has no data but there are records, go to previous page
      if (selectedDepartmentData.value.length == 0 && totalrows.value > 0 && currentPage.value > 1) {
        currentPage.value -= 1;
...
const handleFormSubmit = () => {
  ...
  if (isEditMode.value) {
    axiosEmployee
      .put(`/allDepartments/${currentItem.value.id}`, formData.value)
      .then(() => {
    ...
    axiosEmployee
      .post(`/allDepartments`, formData.value)
      .then(() => {
        sidebarstatus.value.add = false;
        closeSidebar();
        setTimeout(() => {
...
const downloadTemplate = async () => {
  try {
    const response = await axiosEmployee.get('/download-import-template-department', {
      responseType: 'blob'
    });
...
const uploadFile = async () => {
  ...
  try {
    const response = await axiosEmployee.post('/import-department', formData, {
      headers: {
    ...
    if (response.data.message === "No valid data found") {
      toast("The file contains no valid department data", {
        type: "error",
        autoClose: 3000,
      });
    } else {
      closeImportSidebar();
      fetchNoOfData();
      toast(response.data.message || "Departments imported successfully!", {
        type: "success",
        autoClose: 3000,
      });
    }
...
const exportDepartment = async () => {
  ...
  try {
    const response = await axiosEmployee.get("/export-departments", {
      responseType: "blob",
    });
    ...
    const fileName = `departments-${new Date().toISOString().slice(0, 10)}.xlsx`;
...
const confirmDelete = async () => {
  ...
  try {
    const response = await axiosEmployee.delete(`allDepartments/${deleteId.value}`);
    deleteModal.value = false;
    deleteId.value = null;
    toast(response.data.message || "Department and related data deleted successfully!", {
      type: "success",
      autoClose: 1000,
    });
    fetchNoOfData();
